...
int main(){
    ...
    while (flag == 0) {
        ...
        if (amount_players >= 2 && amount_players <= 5) {
            ...
            flag = 1; 
            GameState gameState(amount_players);
            break; 
        } else {
...
void buildDeck(vector<Card*> &deck){
    ...
    for(int c = RED; c < NUM_COLORS; c++){
        for(int n = 0; n < 5; n++){
            Card* temp = new DrawTwoCard((Color)c);
            deck.push_back(temp);
            deck.push_back(temp);
        }
    }
    
    // Create DrawFour Cards
    for(int c = RED; c < NUM_COLORS; c++){
        for(int n = 0; n < 5; n++){
            Card* temp = new DrawFourCard((Color)c);
            deck.push_back(temp);
            deck.push_back(temp);
        }
    }
...
void populateHands(vector<Card*> &deck, vector<vector<Card*>> &hands){
    for(int i = 0; i < hands.size(); i++){
        drawCards(deck, hands.at(i), 7);//updates starting hand
    }
}

void renderHand(vector<Card*> hand){
    if(hand.size() > 0){
        for(int i = 0; i <= 7; i++){ //updates starting hand
